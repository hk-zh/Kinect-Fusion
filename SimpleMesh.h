...
public:
	...
	static SimpleMesh joinMeshes(const SimpleMesh& mesh1, const SimpleMesh& mesh2, Matrix4f pose1to2 = Matrix4f::Identity()) {
		...
		for (int i = 0; i < (int)nVertices1; ++i) {
		...
		for (int i = 0; i < (int)nVertices2; ++i) joinedVertices.push_back(vertices2[i]);

		// Add all faces (the indices of the second mesh need to be added an offset).
		for (int i = 0; i < (int)nTriangles1; ++i) joinedTriangles.push_back(triangles1[i]);
		for (int i = 0; i < (int)nTriangles2; ++i) {
