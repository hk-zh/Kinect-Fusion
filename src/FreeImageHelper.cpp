...
bool FreeImageU16F::SaveImageToFile(float *data, const std::string &filename, unsigned int w, unsigned int h, unsigned int nChannels, bool flipY)
{
	FREE_IMAGE_FORMAT fif = FIF_PNG;
	...
	for (unsigned int j = 0; j < h; j++)
	{
		for (unsigned int i = 0; i < w; i++)
		{
			...
			for (unsigned int c = 0; c < nChannels; c++)
			{
				if (data[nChannels * (w * j + i) + c] == MINF)
				{
					dep = (uint8_t)0;
				}
				else
				{
					dep = (uint8_t)std::min(std::max(0, (int)(13.107f * data[nChannels * (w * j + i) + c])), 255);
				}
				// if (!flipY)
				// 	FreeImage_SetPixelColor(dib, i, h - 1 - j, dep);
				// else
				// 	FreeImage_SetPixelColor(dib, i, j, dep);
				// dib[(int)w*j+i] = (uint8_t)dep
				if (!flipY)
				{
					bits[nChannels * (w * j + i) + c] = dep;
				}
				else
				{
					bits[nChannels * (w * (h - j - 1) + i) + c] = dep;
				}
			}
		}
	}
